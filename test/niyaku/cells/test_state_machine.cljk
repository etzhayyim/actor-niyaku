(ns niyaku.cells.test-state-machine
  "niyaku 荷役 cell state machines — R0 transition coverage (ADR-2606082000). 1:1 port of
  ...
  (:require [clojure.test :refer [deftest is]]
            ...
            niyaku.cells.sts-hoist-cycle.state-machine
            niyaku.cells.trolley-traverse.state-machine
            niyaku.cells.yard-transfer.state-machine
            niyaku.cells.lashing-twistlock.state-machine
            niyaku.cells.manifest-attestation.state-machine
            niyaku.cells.emissions-audit.state-machine))

(ns niyaku.methods.social
  "social.cljc — niyaku DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from niyaku's own
...
(defn draft-observation-post
  "A dry-run observation post over niyaku's own domain (Automated port cargo handling — 9 Pregel cells implementing berth allocation, stowage planning, spreader engagement, ship-to-shore (STS)…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "niyaku R0: live social posting is Council Lv6+ + operator + "
